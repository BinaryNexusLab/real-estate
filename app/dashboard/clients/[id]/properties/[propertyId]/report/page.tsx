...
export default function ReportPage() {
    ...
    if (loadedClient) {
      ...
      if (foundProperty) {
        ...
        const mappedProperty = {
          ...
          price: (foundProperty as any)['Price (AUD)'] ?? 0,
          address: (foundProperty as any)['Full Address'] ?? '',
          suburb: (foundProperty as any).Suburb ?? '',
          state: (foundProperty as any).State ?? '',
          postcode: (foundProperty as any).Postcode ?? '',
          propertyType: (foundProperty as any)['Property Type'] ?? '',
          yearBuilt: (foundProperty as any)['Year Built'] ?? null,
          bedrooms: (foundProperty as any).Bedrooms ?? 0,
          bathrooms: (foundProperty as any).Bathrooms ?? 0,
          carSpaces: (foundProperty as any)['Car Spaces'] ?? 0,
          energyRating: (foundProperty as any)['Energy Rating'] ?? '',
          estimatedRentalValueWeekly:
            (foundProperty as any)['Estimated Rental Value (Weekly)'] ?? 0,
          maintenanceCostAnnual:
            (foundProperty as any)['Maintenance Cost (Annual)'] ?? 0,
        } as unknown as Property;

        setProperty(mappedProperty);

        // Calculate client-specific parameters
        const investmentPeriod = loadedClient.investmentPeriod || 10;
        ...
        const data: ReportData = {
          ...
          clientSalary: loadedClient.salary,
          clientBudget: loadedClient.budget,
  ...
  const handleExportHTML = async () => {
    ...
    link.href = url;
    link.download = `property-analysis-${
      property?.suburb
      ...
      <main className='max-w-4xl mx-auto px-4 py-8'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
          ...
          <Card className='bg-card border-border hover:shadow-lg transition-shadow'>
            ...
            <CardContent className='space-y-4'>
              <p className='text-sm text-muted-foreground'>
                Download a beautifully formatted PDF document with all property
                analysis, financial projections, and investment metrics.
              </p>
          ...
          <Card className='bg-card border-border hover:shadow-lg transition-shadow'>
            ...
            <CardContent className='space-y-4'>
              <p className='text-sm text-muted-foreground'>
                Download an interactive HTML report that can be opened in any
                web browser with full styling and formatting.
              </p>
              <ul className='text-sm text-muted-foreground space-y-2'>
          ...
          <Card className='bg-card border-border hover:shadow-lg transition-shadow'>
            ...
            <CardContent className='space-y-4'>
              <p className='text-sm text-muted-foreground'>
                Download data in CSV format for use in spreadsheets, databases,
        ...
        <Card className='bg-muted/50 border border-border mt-8'>
          <CardContent className='pt-6'>
            <p className='text-sm text-muted-foreground text-center'>
              Reports are generated based on current market data and
              assumptions. Please consult with a qualified financial advisor
